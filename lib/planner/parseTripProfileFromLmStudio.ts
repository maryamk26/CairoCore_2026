...
function buildSystemPrompt(): string {
  return [
    ...
    "Do not infer vibes from companions (kids, family, pets) or broad trip labels like 'family day'. Only set vibes when the user names a vibe, picks a vibe chip, or uses a theme chip that implies one (e.g. Romantic evening → romantic, Historical tour → historical/cultural).",
    "Set categories only when the user names specific place types (mall, museum, park, …) — not from vibes alone (historical/cultural vibe does not mean categories). Put cafe/restaurant in wantsStop only, never in categories.",
