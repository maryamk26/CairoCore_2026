          ...
          <div className="mb-8">
            <div className="flex justify-between items-center mb-3">
              <span className="font-cinzel text-white text-base font-bold drop-shadow-md">
                {visibleQuestions.filter((q) => isAnswered(q.id)).length} of{" "}
                {visibleQuestions.length} answered
              </span>
              <span className="font-cinzel text-white text-base font-bold drop-shadow-md">
                {Math.round(
                  (visibleQuestions.filter((q) => isAnswered(q.id)).length /
            ...
            <div className="w-full bg-[#5d4e37]/80 rounded-full h-3 shadow-lg">
              <div
                ...
                style={{
                  width: `${(visibleQuestions.filter((q) => isAnswered(q.id)).length / visibleQuestions.length) * 100}%`,
                }}
              />
            </div>
          </div>
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <div
                      className={`w-8 h-8 rounded-full bg-[#5d4e37] flex items-center justify-center transition-transform ${
                        isOpen ? "rotate-45" : ""
                      }`}
                    >
                      <svg
                      ...
                      {question.type === "single_choice" && question.options && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          {question.options.map((option) => (
                            ...
                            >
                              <span className="font-cinzel font-semibold">{option.label}</span>
                            </button>
                          ))}
                        </div>
                      )}
                          ...
                          <input
                            ...
                            style={{
                              background: `linear-gradient(to right, #d4af37 0%, #d4af37 ${
                                ((((currentAnswer as number) || question.min || 0) -
                                  (question.min || 0)) /
                                  ((question.max || 100) - (question.min || 0))) *
                                100
                              }%, #5d4e37 ${
                                ((((currentAnswer as number) || question.min || 0) -
                                  (question.min || 0)) /
          ...
          <div className="text-center">
            ...
            >
              {allAnswered
                ...
                : `Answer ${visibleQuestions.length - visibleQuestions.filter((q) => isAnswered(q.id)).length} More Questions`}
            </button>
          </div>
        </div>
      </div>
